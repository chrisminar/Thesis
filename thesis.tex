...
The solver uses a staggered grid, unlike Luo et al. which uses a co-located grid, causing the process to have three variations; one for pressure nodes and ones for u and v velocity nodes.
Field values are first interpolated at an image point (IP) outside the body using the surrounding field values and the boundary condition. Figure~\ref{Fig:Ghost node extrapolation} depicts the extrapolation for two u velocities.
...
\begin{figure}[htb]
	...
	\caption{A detailed schematic of field value extrapolation to a ghost node.}
...
If one of the four field values used to calculate the IP is inside the body, as seen in the extrapolation for GN 1 in Figure~\ref{Fig Ghost node extrapolation}, then that node is moved to the BI and the boundary condition is used.
...
To satisfy the no slip condition at the body, a Dirichlet boundary condition equal to the body velocity is used when extrapolation for velocity GNs. 
The Neumann boundary condition, Equation~\eqref{eq:Neumann}, is used to approximate pressure.
A system of equations is set up to solve for the $a$ coefficients in Equation~\eqref{eq:Interpolation}.
For velocity, each of the four nodes surrounding the IP form one of the four required equations.
For pressure, nodes on the body get plugged into Equation~\eqref{eq:Neumann Node}, which is simply the derivative of Equation~\eqref{eq:Interpolation}. 
\begin{align}
